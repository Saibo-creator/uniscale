...
(This needs to be run from the repo root directory, where `scripts/` and `experiments/` are located, so that the job script can find the config file and the training script.)
...
Step 5 (Debugging Multi-Node Training):

Sometimes multi-node training can encounter issues that are not present in single-node runs. To debug these, you can check the logs generated by SLURM for each node. They are under
```
less log/slurm-<JOBID>.out
less log/slurm-<JOBID>.err
```
where `<JOBID>` is the SLURM job ID. Look for any error messages or stack traces that can help identify the issue. 


Step 6 (Custom tokenizer):

If you want to train a model with a custom tokenizer, just use any path to a tokenizer directory in the shared storage (e.g. `/capstor/store/cscs/swissai/a139/saibo_uniscale/out/tokenizers/bpe_apertus_128k`) and pass it as an environment variable when submitting the job:

```bash
sbatch --nodes=2 --account=a139 -p normal  --time=01:30:00 --export=ALL,MODEL_SIZE=100M,TOKENIZER_PATH=/capstor/store/cscs/swissai/a139/saibo_uniscale/out/tokenizers/unigram_apertus_128k scripts/slurm_train.sbatch
```
